...
public class LanguageServerWrapper implements Disposable {
    ...
    record ConnectFileData(@Nullable Document document, @Nullable String documentText, @Nullable String languageId, boolean waitForDidOpen) {}
                    ...
                    .thenAccept(res -> {
                        ...
                        initializeFuture.thenRunAsync(() -> {
                            for (VirtualFile fileToReconnect : filesToReconnect) {
                                connect(fileToReconnect, new ConnectFileData(null, null, null,true));
                            }
                        });
     ...
     * @param connectFileData the document of the file and null otherwise. In the null case, the document will be retrieved from the file
    ...
    CompletableFuture<@Nullable LanguageServer> connect(@NotNull VirtualFile file,
                                                        @NotNull ConnectFileData connectFileData) {
        ...
        boolean waitForDidOpen = connectFileData.waitForDidOpen();
        ...
        return initializeFuture.thenComposeAsync(theVoid -> {
            ...
            Document optionalDocument = connectFileData.document();
            ...
            synchronized (connectedDocuments) {
                ...
                DocumentContentSynchronizer synchronizer = createDocumentContentSynchronizer(fileUri.toASCIIString(), file, document, connectFileData.documentText(), connectFileData.languageId());
