...
public class DAPClient implements IDebugProtocolClient, Disposable {
    ...
    public CompletableFuture<Void> connectToServer(@NotNull ProgressIndicator indicator) {
        TracingMessageConsumer tracing = isShowDapTraces() ? new TracingMessageConsumer() : null;

        UnaryOperator<MessageConsumer> wrapper = consumer -> {
            MessageConsumer result = consumer;
            if (tracing != null) {
                result = message -> {
                    String log = tracing.log(message, consumer, ServerTrace.verbose);
    ...
    private boolean isShowDapTraces() {
        // TODO: add a custom action to the DAP console to manage that.
        return true;
    }
    ...
    public CompletableFuture<Void> startDebugging(StartDebuggingRequestArguments args) {
        ...
        DAPClient client = new DAPClient(debugProcess, parameters, debugMode, debuggingType, this);
