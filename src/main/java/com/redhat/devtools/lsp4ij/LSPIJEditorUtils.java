...
public final class LSPIJEditorUtils {
    ...
    private static boolean isTextMateFile(@NotNull PsiFile file) {
        return file.getFileType() instanceof TextMateFileType;
    }
    ...
    public static Set<Character> getQuoteCharacters(@NotNull PsiFile file) {
        Set<Character> quoteCharacters = file.getFileType() instanceof AbstractFileType ? getAbstractFileTypeQuoteCharacters(file) :
                file instanceof TextMateFile ? getTextMateQuoteCharacters(file) :
                        null;
    ...
    private static Set<Character> getAbstractFileTypeQuoteCharacters(@NotNull PsiFile file) {
        return !(file.getFileType() instanceof AbstractFileType) ? null : DEFAULT_QUOTE_CHARACTERS;
    }

    @Nullable
    private static Set<Character> getTextMateQuoteCharacters(@NotNull PsiFile file) {
        /* TODO: See TextMate versioning comment above
        Editor editor = file instanceof TextMateFile ? LSPIJUtils.editorForElement(file) : null;
        TextMateScope scope = editor != null ? TextMateEditorUtils.getCurrentScopeSelector((EditorEx) editor) : null;
    ...
    public static Map<Character, Character> getBracePairs(@NotNull PsiFile file) {
        Map<Character, Character> bracePairs = file.getFileType() instanceof AbstractFileType ? getAbstractFileTypeBracePairs(file) :
                file instanceof TextMateFile ? getTextMateBracePairs(file) :
                        null;
        if (bracePairs == null) {
