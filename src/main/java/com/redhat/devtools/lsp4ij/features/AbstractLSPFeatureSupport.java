...
public abstract class AbstractLSPFeatureSupport<Params, Result> {
     ...
     */
    @Nullable
    public abstract CompletableFuture<Result> getFeatureData(Params params);
    ...
    /**
     * Cancel previous LSP requests and load the LSP requests for all language servers applying to a given Psi file or project.
     *
     * @param serversFuture the future of language servers.
     * @param params        the LSP parameters expected to execute LSP requests.
     * @return the LSP response results.
     */
    protected synchronized CompletableFuture<Result> load(CompletableFuture<List<LanguageServerItem>> serversFuture, Params params) {
        if (isValidLSPFuture()) {
            return future;
        }
        // Cancel previous LSP requests future
        cancel();
        // Create a new cancellation support for the data loading
        cancellationSupport = new CancellationSupport();
        // Chain the servers future with doLoadData
        // The serversFuture is not stored in this.future, only the result of thenComposeAsync is stored
        future = serversFuture
                .exceptionally(throwable -> {
                    return null;
                })
                .thenComposeAsync(languageServers -> doLoadData(languageServers, params, cancellationSupport));
        return future;
    }

    /**
     * Returns the language servers that support this feature.
     *
     ...
     * @return the LSP response results.
     */
    protected abstract CompletableFuture<Result> doLoadData(List<LanguageServerItem> languageServers, Params params, CancellationSupport cancellationSupport);
