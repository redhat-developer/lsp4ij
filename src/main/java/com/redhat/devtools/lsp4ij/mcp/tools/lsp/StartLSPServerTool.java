...
/**
 * MCP tool to start a language server.
 */
public class StartLSPServerTool extends MCPToolBase {

    private static final Logger LOGGER = LoggerFactory.getLogger(StartLSPServerTool.class);
        ...
        return """
            {
              ...
              "required": ["serverId"],
              "description": "Start a language server by its ID"
            }
            """;
    }
    ...
    public McpSchema.CallToolResult execute(
        ...
        try {
            ...
            LanguageServerManager.getInstance(project).start(serverId);

            Map<String, Object> result = new HashMap<>();
            result.put("serverId", serverId);
            result.put("status", "started");
            result.put("message", "Language server start initiated");
