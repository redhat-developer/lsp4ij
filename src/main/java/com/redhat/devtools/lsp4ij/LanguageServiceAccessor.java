...
public class LanguageServiceAccessor implements Disposable {
    ...
    CompletableFuture<@NotNull List<LanguageServerItem>> getLanguageServers(@NotNull VirtualFile file,
        ...
        boolean waitForDidOpen = !ApplicationManager.getApplication().isWriteAccessAllowed();
        Document document = ApplicationManager.getApplication().isReadAccessAllowed() ? LSPIJUtils.getDocument(file) : null;

        Map<LanguageServerWrapper, LanguageServerWrapper.ConnectFileData> connectFileData = new HashMap<>();
        for (var ls : matchedServersNow) {
            ...
            if (uri != null && !ls.isConnectedTo(uri)) {
                if(document != null) {
                    ...
                    var data = new LanguageServerWrapper.ConnectFileData(document, text, languageId, true);
                    connectFileData.put(ls, data);
                }
            }
        }
                                        ...
                                        return wrapper.getInitializedServer()
                                                .thenComposeAsync(server -> {
                                                    if (server != null &&
                                                        ...
                                                        var data = connectFileData.get(wrapper);
                                                        if (data == null) {
                                                            data = new LanguageServerWrapper.ConnectFileData(document, null, null, waitForDidOpen);
                                                        }
                                                        return wrapper.connect(file, data);
                                                    }
                                                    return CompletableFuture.completedFuture(null);
                                                }).thenAccept(server -> {
                                                    if (server != null) {
