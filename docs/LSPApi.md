...
| boolean supportsGotoClass() | Returns `true` if the LSP feature is supported and efficient enough to support the IDE's Go To Class action which may be invoked frequently and `false` otherwise.                       | `false`                                |
